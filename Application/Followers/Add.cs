    ...
    public class Add
    {
        ...
        public class Handler : IRequestHandler<Command>
        {
            ...
            public async Task<Unit> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                var followings = await _context.Followings.SingleOrDefaultAsync(x => x.ObserverId == observer.Id && x.TargetId == target.Id);

                if(followings!=null) throw new RestException(HttpStatusCode.BadRequest, new {User = "You are already following this user"});

                if(followings == null)
                {
                    followings = new UserFollowings
                    {
                        Observer = observer,
                        Target = target
                    };
